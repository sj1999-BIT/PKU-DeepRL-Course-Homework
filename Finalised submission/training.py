...
def single_epoch_update(policy_net: PolicyNetwork, value_net: ValueNetwork, data_processor: TrajectoryProcessor,
                        envs: [gym.Env], trajectory_length=1000, batch_size=64):
    """
    ...
    """
    trajectory_tensor = data_processor.generate_data(envs, policy_net, value_net, trajectory_length)
...
def ppo_training(trajectory_num=10, trajectory_len=1000):
    ...
    # number of environment links to how many trajectories we are collecting
    envs = [gym.make(env.unwrapped.spec.id) for _ in range(trajectory_num)]
    ...
    while True:
        ...
        single_epoch_update(pNet, vNet, processor, envs, trajectory_len, batch_size=64)

        # record the new reward obtained from the updated model
        reward = get_reward(weight_path="./data/Policy_nn_weight.pth")
...
if __name__ == "__main__":
    # ppo_training(trajectory_num=100, trajectory_len=100)  # 100 trajectories of 100 steps
    ppo_training(trajectory_num=10, trajectory_len=1000)  # 10 trajecories of 1000 steps
